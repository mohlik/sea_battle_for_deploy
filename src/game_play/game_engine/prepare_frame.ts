class PrepareFrame {
    ...
    is_posible(x: number, y: number, strong: boolean) {
        let possible
    ...
    is_posible_ship(x: number, y: number, x_l: number, y_l: number) {
        let possible = true;
    ...
    update_field() {
        this.clear_field();
        this.ships.forEach((ship_data: any) => {
            this.add_ship(ship_data, true);
        });
    }

    remove_ship(params: any) {
    ...
    add_ship(params: any, is_update?: boolean) {
        let {x, y, x_l, y_l} = params;
