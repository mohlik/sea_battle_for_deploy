class CustomButton extends Phaser.GameObjects.Container {
    out_img: Phaser.GameObjects.Image
    over_img: Phaser.GameObjects.Image
    down_img: Phaser.GameObjects.Image
    callback: Function
    was_down: boolean
    clickable: boolean

    constructor(scene: Phaser.Scene, params: CustomButtonConfig) {
        super(scene);
        this.init(params);
    }

    init(params: CustomButtonConfig): void {
        this.clickable = true;
        this.x = params.x;
        this.y = params.y;
    ...
    create_interactive(over_default: boolean, down_default: boolean): void {
        this.out_img.setInteractive({useHandCursor: true});
		this.set_out(over_default || down_default);
		this.out_img.on('pointerdown', () => {
            if(this.clickable) this.set_down(down_default); 
		});
		this.out_img.on('pointerover', () => {
            if(this.clickable) this.set_over(over_default);
		});
		this.out_img.on('pointerout', () => { 
            if(this.clickable) this.set_out(over_default || down_default);
		});
    ...
    set_handler(_callback: Function) {
