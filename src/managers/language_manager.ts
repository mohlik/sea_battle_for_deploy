class LanguageManager {
    ...
    load_lang(on_complete: Function): void {
    ...
    get_user_lang(): string {
        return 'en_US'  // server
    }

    update_user_lang(): void {
        console.log('to server', this.lang);
    }

    get_text_for_id(text_id: string) {
        let lang_table: any;
        if(this.lang in this.loaded_languages) {
    ...
    add_text(obj: CustomText, text_id: string): void {
        let text_obj = {
            text_object: obj,
            text_id
        };
